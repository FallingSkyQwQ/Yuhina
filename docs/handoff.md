...
- [x] **M3 账号（协议层）**：真实协议链路已验证于 2026-08-30（`feature/m3-auth`，本地 `main` 派生）。证据：`rust/rust/tests/real_auth_test.rs`（`#[ignore]`，`cargo test --test real_auth_test -- --ignored --nocapture`，4/4 通过）。① **微软 loopback 链**：`begin_microsoft_login` 起 `tiny_http` 监听 `127.0.0.1:<随机高位端口>/callback`，真实 client_id `ff0aea8c-…79b` 注入授权 URL；注入假 code + 正确 state 的回调被真实 token 端点拒绝，`poll` 返回可读错误（`AuthExpired`：`invalid_grant AADSTS7000012 …`），不 panic 不挂起；`cancel` 干净释放端口。② **LittleSkin 真实端点**：错误密码 → `Auth` 错误，message 含服务器原文 `用户名或密码错误。Incorrect username or password.`（协议与 `/authserver/authenticate` URL 正确）；未知 uuid profile → HTTP 404，`fetch_skin` 安全返回 `None`（`/sessionserver/session/minecraft/profile/{uuid}` 规则经 `/hasJoined`(204) 探活确认路由正确）。
...
| 真实微软 loopback 链 + 假 code 错误路径 / LittleSkin 真实错误路径 | D | 需外网；`rust/rust/tests/real_auth_test.rs`（`--ignored`，M3 已验证 4/4）。真人授权成功换取与小皮肤真实账号登录不进 CI，需手动 E2E |
